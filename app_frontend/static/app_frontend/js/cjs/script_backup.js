...
fillCategoriesOptionsSelectMenu();
  ...
  $("#product-type").change(function () {
    ...
    if(value === "approved"){
      ...
      get_products_and_fill_list(search_by=last_used_filter_category, search=last_used_filter_category_value, sort=selected_sort);
      
      document.querySelector("#inventory-transactions-div").style.display = "block";
      ...
      document.querySelector("#inventory-transactions-div").style.display = "none";
      document.querySelector("#approve-product-btn-div").style.display = "block";
      resetFormFields();
  ...
  function get_products_and_fill_list(search_by, search, sort="asc", group_val=null) {
    
    last_used_filter_category = search_by;
    last_used_filter_category_value = search;

    let URL_GET_PRODUCTS = "//" + BASE_URL +  "/api/get-products-with-filters/?sort="+sort;

    if(selected_group_id !== null){
      URL_GET_PRODUCTS = URL_GET_PRODUCTS + "&group=" + selected_group_id;
    }

    if (search === null || search === 0 || search_by===undefined || search_by===null) {
  ...
  function getProductDetailsAndFillProductFormData(id) {
    ...
    $.ajax({
      ...
      success: function (json_data) {
        ...
        selected_sds_record_id = json_data["data"]['list']["SDSRecords"]["list"][0]["id"]
        ...
        if (json_data["success"] === true) {
          ...
          // );
          document.querySelector("#product-reorder-level").value = json_data["data"]["list"]["ReorderLevel"]
      ...
      success: function (json_data) {
        ...
        if (json_data["success"] === true) {
          ...
          for (let i = 0; i < json_data["data"]["count"]; i++) {
            ...
            $(td_date_input).attr("class", "stock-transaction-change-listener-class");
            ...
            if (json_data["data"]["list"][i]["DateTransaction"] !== null) {
              ...
              $(td_date_input).val(json_data["data"]["list"][i]["DateTransaction"]);
            }
            ...
            $(td_lot_number_input).attr("class", "stock-transaction-change-listener-class");
            $(td_lot_number_input).val(json_data["data"]["list"][i]["LotNumber"]);
            ...
            $(td_description_input).attr("class", "stock-transaction-change-listener-class");
            $(td_description_input).val(json_data["data"]["list"][i]["Description"]);
            ...
            $(td_units_input).attr("class", "stock-transaction-change-listener-class");
  ...
  group_search_input.addEventListener("blur", ()=>{
    ...
    // alert(this.value)
    if (!this.value){
      get_products_and_fill_list(last_used_filter_category, last_used_filter_category_value, selected_sort);
      // alert("here");
    }
  })
  ...
  // function updateSDSField(name_field, value_field, id_product, id_sds){
  //   const formData = new FormData();
  //   if(id_sds === null){
  //     formData.append("IDRecord", current_selected_product["id"]);
  //     formData.append("SDSOnFile", document.querySelector("#sds-on-file-checkbox").value);
  //     formData.append("SDSCasNo", document.querySelector("#sds-cas-num").value);
  //     formData.append("SDSComponent", document.querySelector("#sds-cas-num").value);
  //     formData.append("SDSNameURL", document.querySelector("#sds-cas-num").value);
  //     formData.append("SDSLocationURL", document.querySelector("#sds-cas-num").value);
  //     $.ajax({
  //       type: "POST",
  //       url: URL_CREATE_SDS_RECORD,
  //       data: formData,
  //       processData: false,
  //       contentType: false,
  //       beforeSend: function (xhr) {
  ...
  //         if (json_data["success"] === true) {
  //           console.log(json_data["data"]);
  //           showToast("SDS for product Created ", (type = "success"));
  ...
  //       },
  //       error: function (json_data) {
  //         console.log("ERROR CREATING SDS RECORD");
  //         showToast("Failed to approve the product", type="warning");
  //       },
  //     });
  //   }
  ...
  function resetFormFields(){
    ...
    document.querySelector("#product-name-heading").innerHTML = "Select Product..";
    ...
    document.querySelector("#product-image").src = 'https://image.made-in-china.com/2f0j00yosfPngahbcQ/Promotion-Oversized-Product-Package-Cardboard-Dummy-Boxes-Bin-for-Storage.jpg';
